...
int main()
{
    ...
    char filename[MAX_FILENAME];                                                                     // Указальнік на масіў адфільтраваных дзяцей.
    choose_input(&children, &number_of_children);
    output_Child(children, number_of_children);
    choose_output(filename);
    write_children_to_file(filename, children, number_of_children, "wb");
    ...
    write_children_to_file(filename, children_with_target_illness, result_count, "ab");
