...
#let appendix(letter, title) = {
  ...
  // Создаем невидимый заголовок. 
  // Он попадет в оглавление, но не займет визуального места на листе.
  [
    #show heading: none
    #heading(numbering: none)[ПРИЛОЖЕНИЕ #letter]
  ]
  
  // Рисуем нужный текст без наложений
  align(center)[
...
#show raw: set text(size: 11pt, font: "Courier New")

// Разрешаем блокам с кодом (figure kind: raw) разрываться между страницами
#show figure.where(kind: raw): set block(breakable: true)

// Чтобы подписи к коду назывались "Листинг", а не "Рисунок"
#show figure.where(kind: raw): set figure(supplement: [Листинг#show figure.where(kind: raw): set figure(supplement: [Листинг])])

// Отключаем красную строку для блоков кода в приложениях
#set par(first-line-indent: 0pt)


// === ПРИЛОЖЕНИЕ В ===
#appendix("В", "Код файла header.h")

#figure(
  raw(read("header.h"), lang: "c", block: true),
  caption: [Исходный код заголовочного файла header.h],
) <listing-header>


// === ПРИЛОЖЕНИЕ Г ===
#appendix("Г", "Код файла func.c")

#figure(
  raw(read("func.c"), lang: "c", block: true),
  caption: [Исходный код файла реализации func.c],
) <listing-func>


// === ПРИЛОЖЕНИЕ Д ===
#appendix("Д", "Код файла main.c")
